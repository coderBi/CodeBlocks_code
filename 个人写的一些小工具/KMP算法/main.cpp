/*
    思路：
        ...
        ②：在查找的目标序列中，从index为0 开始向右移动，一直到某一位跟相应的要找的子序列中不同，就查询next数组，子序列回溯相应的位置，又从子序列的 next[i] 位置开始比对
        ③：next数组的建立：
        ...
        ④：如果对于每一个长度都去对比所有前缀表达式与后缀表达式，这个复杂度也是糟糕的，进一步分析可以如下产生next数组
            ---next[0] = -1, 原因是如果第一个就不符合，目标序列就直接向前移动而子序列不动，那么回溯的长度就是 “-1”
            ---next[1] = 0, 原因是如果索引是 1 的不符合，如果进行回溯，最好的结果是 回溯到起始的索引 0。这个没有意义，所以不进行回溯而是从当前位就行新的查找
            ---i > 1, 假设next[i] = k,如果查找的子序列 subStr[i] = subStr[k] (第 i + 1位 与 第 k + 1 位也相同),那么 next[i + 1] = k + 1; 如果  subStr[i] != subStr[k]。
                那么这个问题的思路可以改成这样：前 k位中，第 k+1位 subStr[k] = 目前的 subStr[i] 的情况下(这前k+1个字符不满足需求) 求 next[k]。也即是如果subStr[i] != subStr[k],就将k 调整为 next[k]
...
int getSubStringIndex(string s1, string s2){
    //得到next数组
    int * next = new int[s2.size()]; //定长的数组在c++11之后最好用array而不用vector，这特更高效。这由于mingW的版本没有到C++11所以使用自己new的方式
    next[0] = -1; next[1] = 0;
